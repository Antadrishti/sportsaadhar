...
/// Face verification screen specifically for height test
class HeightFaceVerificationScreen extends StatefulWidget {
  final PhysicalTest test;
  final String? videoPath; // Video path from recording screen

  const HeightFaceVerificationScreen({
    super.key,
    required this.test,
    this.videoPath,
  });
  ...
  @override
  State<HeightFaceVerificationScreen> createState() => _HeightFaceVerificationScreenState();
}

class _HeightFaceVerificationScreenState extends State<HeightFaceVerificationScreen> {
  ...
  Future<void> _analyzeTest() async {
    try {
      ...
      debugPrint('🔍 Starting height measurement from video: ${widget.videoPath}');
      final result = await _heightService.measureHeightFromVideo(widget.videoPath!);
      ...
      debugPrint('❌ Error in _analyzeTest: $e');
      if (mounted) {
        setState(() => _isProcessing = false);
        
        // Even on error, navigate to result screen with error status
        final state = context.read<AppState>();
        final user = state.user;
        ...
        if (user != null) {
          final testResult = TestResultModel(
            ...
            date: DateTime.now(),
            measuredHeight: null,
            registeredHeight: user.height,
            isHeightVerified: false,
          );

          debugPrint('🚀 Navigating to result screen after error');
          Navigator.pushReplacementNamed(
            context,
            '/height-result',
            arguments: testResult,
