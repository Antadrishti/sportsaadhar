...
/// Face verification screen for shuttle run test
class ShuttleRunFaceVerificationScreen extends StatefulWidget {
  final PhysicalTest test;

  const ShuttleRunFaceVerificationScreen({
    super.key,
    required this.test,
  });

  @override
  State<ShuttleRunFaceVerificationScreen> createState() =>
      _ShuttleRunFaceVerificationScreenState();
}

class _ShuttleRunFaceVerificationScreenState
    extends State<ShuttleRunFaceVerificationScreen> {
  ...
  Future<void> _handleVideoSaved(String videoPath) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        if (result.isMatch) {
          // Success! Navigate to shuttle run setup screen
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                  '✅ Face Verified! Proceeding to setup...'),
              backgroundColor: Colors.green,
              duration: Duration(seconds: 2),
            ),
          );

          // Navigate to setup screen after a short delay
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          const Text(
                            'Record a short video of your face to verify your identity before starting the shuttle run test.',
                            style: TextStyle(
