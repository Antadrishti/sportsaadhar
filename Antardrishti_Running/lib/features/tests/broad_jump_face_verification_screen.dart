...
/// Face verification screen for broad jump test
class BroadJumpFaceVerificationScreen extends StatefulWidget {
  final PhysicalTest test;
  final double userHeightCm;
  final String? videoPath; // Video path from recording screen

  const BroadJumpFaceVerificationScreen({
    super.key,
    required this.test,
  ...
  @override
  State<BroadJumpFaceVerificationScreen> createState() =>
      _BroadJumpFaceVerificationScreenState();
}

class _BroadJumpFaceVerificationScreenState
    extends State<BroadJumpFaceVerificationScreen> {
  ...
  Future<void> _analyzeTest() async {
    try {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('✅ Face Verified! Analyzing test...'),
      ...
      // Measure broad jump from video
      final result = await _broadJumpService.measureBroadJumpFromVideo(
        widget.videoPath!,
        widget.userHeightCm,
      );
      ...
      if (result.success && result.jumpDistanceM != null) {
        debugPrint('🎯 Broad Jump Distance: ${result.formattedDistanceM}');
        ...
        final testResult = TestResultModel(
          ...
          distance: result.jumpDistanceM ?? 0,
          timeTaken: 0,
          speed: 0,
          date: DateTime.now(),
          jumpHeight: result.jumpDistanceCm,
          jumpType: 'broad',
        );
        ...
        // Measurement failed
        String errorMsg = result.errorMessage ?? 'Failed to measure broad jump';
