...
/// Face verification screen for sit and reach test
class SitAndReachFaceVerificationScreen extends StatefulWidget {
  final PhysicalTest test;
  final String? videoPath; // Video path from recording screen

  const SitAndReachFaceVerificationScreen({
    super.key,
    required this.test,
    this.videoPath,
  });
  ...
  @override
  State<SitAndReachFaceVerificationScreen> createState() =>
      _SitAndReachFaceVerificationScreenState();
}

class _SitAndReachFaceVerificationScreenState
    extends State<SitAndReachFaceVerificationScreen> {
  ...
  Future<void> _analyzeTest() async {
    try {
      ...
      final result = await _flexibilityService.measureFlexibilityFromVideo(widget.videoPath!);
      ...
      if (result.success && result.bestAngle != null) {
        ...
        if (result.framesWithPose == 0 || result.framesWithPose == null) {
          errorMsg += '\n\nTip: Make sure your ear, hip, and knee are visible in side view throughout the video.';
        }
