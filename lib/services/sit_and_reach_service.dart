      ...
      for (int i = 0; i < frames.length; i++) {
        try {
          ...
          if (poses.isNotEmpty) {
            ...
            // Get landmarks (center point logic because we don't know side)
            // But sit and reach is usually viewed from side.
            // Let's check left side first, then right if left is not visible/confident
            PoseLandmark? ear = pose.landmarks[PoseLandmarkType.leftEar];
            PoseLandmark? hip = pose.landmarks[PoseLandmarkType.leftHip];
            PoseLandmark? knee = pose.landmarks[PoseLandmarkType.leftKnee];

            bool useRight = false;

            if (ear == null || hip == null || knee == null || 
                ear.likelihood < 0.5 || hip.likelihood < 0.5 || knee.likelihood < 0.5) {
                // Try right side
                ear = pose.landmarks[PoseLandmarkType.rightEar];
            ...
            if (ear != null && hip != null && knee != null &&
              ...
              if (angle < bestAngle) {
                bestAngle = angle;
                debugPrint('✅ New best angle: ${bestAngle.toStringAsFixed(1)}° at frame $i (${useRight ? "Right" : "Left"})');
              }
            }
          }
        } catch (e) {
      ...
      for (int i = 0; i < maxFrames; i++) {
        ...
        if (uint8list != null) {
          ...
          framePaths.add(framePath);
             ...
             // If we stop getting frames, break early (video ended)
             break;
        }
      }
    } catch (e) {
